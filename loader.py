...
def fast_resample_sharp(volume, msw_res=0.5, chh_res=0.9, target_shape=(64, 64, 72), sharpen=True):
    scaling_factor = msw_res / chh_res  
    in_center = np.array(volume.shape) / 2.0
    out_center = np.array(target_shape) / 2.0
    ...
    if sharpen:
        ...
        resampled = resampled + 0.6 * (resampled - blurred)
        ...
        for f in tqdm(all_potential, desc="Caching Volumes"):
            ...
            try:
                ...
                if self.resample_fn is not None:
                    # CHH MODE (0.9mm -> 0.5mm)
                    native_crop_shape = tuple(int(round((ts * 0.5) / 0.9)) for ts in target_shape)
                    ...
                    standardized_data = self.resample_fn(raw_patch, target_shape=target_shape, sharpen=True)
                    m_patch = self.resample_fn(mask_patch, target_shape=target_shape, sharpen=False)
                ...
                # 7. MANDATORY AXIAL ALIGNMENT
                # If your debug plot showed butterflies with [:, :, mid_z], 
                # we must ensure that internal storage matches that exact order.
